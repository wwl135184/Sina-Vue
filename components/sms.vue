...
<template>
	<div>
		<section id="sms" class="main "><input type="hidden" name="act" value="sendcode">
			<div class="mod h5_select_long"><span id="country_code">{{sel}}</span><span id="country_name" >中国大陆</span>
				<select id="sms_select_area" v-model="sel">
			...
			<div class="mod mod_more h5_input input_change_type"><input id="sms_pwd_input" disableemoticoninput="true" type="text" name="pwd" maxlength="16" placeholder="6~16位数字或字母，区分大小写" validate="pwd" @focus = "pwdfocus" @blur = "pwdblur">
	...
	export default{
		...
		methods: {
			...
			showhidden: function(){
				this.show = !this.show;
				if(this.show){
					$("#sms_pwd_input").html("显示")
					$("#sms_pwd_input").attr("type", "password");
				}else{
					$("#sms_pwd_input").html("隐藏")
